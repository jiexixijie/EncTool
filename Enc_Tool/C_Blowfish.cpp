...
int C_Blowfish::Init_Enc_evp_ctx(EVP_CIPHER_CTX& ctx, int type, char* key, char* iv) {
...
int C_Blowfish::Init_Dec_evp_ctx(EVP_CIPHER_CTX& ctx, int type, char* key, char* iv) {
	EVP_CIPHER_CTX_init(&ctx);
	const EVP_CIPHER* ciphertype;
	switch (type)
	{
