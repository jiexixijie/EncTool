...
void CAESPage::OnBnClickedButton1()
{
	...
	//文件加解密
		...
		int result = a.EncFile(FileR_path, FileW_path, key_c, iv_c, AES_TYPE_List[m_EncType_cbox.GetCurSel()]);
		switch (result)
		{
		case ReadError:
...
void CAESPage::OnBnClickedButton2()
{
	...
	if (!m_EncData.IsFile()) {
		...
		int result = a.DecFile(FileR_path, FileW_path, key_c, iv_c, AES_TYPE_List[m_EncType_cbox.GetCurSel()]);
		switch (result)
		{
		case ReadError:
			MessageBox(_T("文件读取失败"));
